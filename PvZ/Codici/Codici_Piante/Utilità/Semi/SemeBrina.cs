			...
			lock (gioco.LockZombie)
			{
				if (fila != 5)
                {
					for (int i = 0; i < gioco.Mappa_zombie[fila].Count; i++)
						if (Zaux == null && Tocca(gioco.Mappa_zombie[fila][i]))
							try
							{
								lock (gioco.LockZombie)
								{
									if (gioco.Mappa_zombie[fila][i] != null)
										Zaux = gioco.Mappa_zombie[fila][i];
								}
							}
							catch (Exception) { }
						else if (Zaux != null)
							if (Zaux.sprite.Position.X > gioco.Mappa_zombie[fila][i].sprite.Position.X && Tocca(gioco.Mappa_zombie[fila][i]))
								try
								{
									if (gioco.Mappa_zombie[fila][i] != null)
										Zaux = gioco.Mappa_zombie[fila][i];
